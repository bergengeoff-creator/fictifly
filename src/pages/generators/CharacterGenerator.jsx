...
const ALL_FIELDS = [
  { key: 'ageRange',       emoji: '🎂', label: 'Age Range',      example: 'Late 30s' },
  { key: 'profession',     emoji: '💼', label: 'Profession',     example: 'Disgraced architect' },
  { key: 'background',     emoji: '🌍', label: 'Background',     example: 'Grew up in a fishing village, moved to the city at 16' },
  { key: 'motivation',     emoji: '🎯', label: 'Motivation',     example: 'Wants to reconnect with an estranged daughter' },
  { key: 'hobbies',        emoji: '🎸', label: 'Hobbies',        example: 'Collects vintage maps, plays chess badly' },
  { key: 'personality',    emoji: '🧠', label: 'Personality',    example: 'Quietly stubborn, disarmingly warm with strangers' },
  { key: 'talents',        emoji: '✨', label: 'Talents',        example: 'Exceptional memory, can mimic any accent' },
  { key: 'flaw',           emoji: '💔', label: 'Flaw',           example: 'Cannot accept help from anyone' },
  { key: 'secret',         emoji: '🤫', label: 'Secret',         example: 'Changed their name 12 years ago and never told anyone why' },
  { key: 'voice',          emoji: '🗣️', label: 'Voice',          example: 'Speaks in understatements, rarely finishes sentences' },
  { key: 'physicalDetail', emoji: '👁️', label: 'Physical Detail', example: 'Always wears the same worn leather bracelet' },
  { key: 'name',           emoji: '📛', label: 'Name',           example: 'Optional — leave off for mystery' },
];
...
const FieldRow = ({ field, value, locked, onToggleLock, onDeselect, onRefresh, refreshing }) => (
  <div style={{
    ...
    <span style={{ fontSize: '1.1rem', marginTop: '0.1rem', minWidth: '1.4rem', textAlign: 'center' }}>{field.emoji}</span>
...
Write a single, vivid paragraph (3–5 sentences) that captures who this person is — their essence, contradictions, and what makes them compelling as a fictional character. Write in third person. Be literary and specific. Do not list the traits — synthesise them into a portrait.`;
  };
  ...
  const copySheet = () => {
    const lines = [...activeFields].map(key => {
      const f = ALL_FIELDS.find(f => f.key === key);
      return `${f.emoji} ${f.label.toUpperCase()}: ${values[key] || '—'}`;
    });
                    ...
                    <div>
                      {ALL_FIELDS.filter(f => c.character_data?.[f.key]).map(f => (
                        <div key={f.key} style={{ display: 'flex', gap: '0.75rem', padding: '0.5rem 0', borderBottom: `1px solid ${B.sandMid}` }}>
                          <span style={{ fontSize: '0.9rem', minWidth: '1.3rem', textAlign: 'center' }}>{f.emoji}</span>
                          <div>
                            <Label>{f.label}</Label>
