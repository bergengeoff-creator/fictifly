...
async function getUserFromToken(req) {
  ...
  if (!token) {
    console.error('getUserFromToken: no token in Authorization header');
    return null;
  }
  ...
  try {
    ...
    if (error || !user) {
      console.error('getUserFromToken: auth.getUser rejected token:', error);
      return null;
    }
    return user;
  } catch (err) {
    console.error('getUserFromToken: threw exception:', err);
    return null;
  }
}
